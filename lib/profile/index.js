...
function defaults ( ) {
  return /* profile */ {
    max_iob: 6 // if max_iob is not provided, will default to zero
    ...
    , high_temptarget_raises_sensitivity: true // raise sensitivity for temptargets >= 101.  synonym for exercise_mode
    , low_temptarget_lowers_sensitivity: true // lower sensitivity for temptargets <= 99.
    ...
    , maxSMBBasalMinutes: 50 // maximum minutes of basal that can be delivered as a single SMB with uncovered COB
    , maxUAMSMBBasalMinutes: 40 // maximum minutes of basal that can be delivered as a single SMB when IOB exceeds COB
    , SMBInterval: 2 // minimum interval between SMBs, in minutes.
    , bolus_increment: 0.05 // minimum bolus that can be delivered as an SMB
    , maxDelta_bg_threshold: 0.3 // maximum change in bg to use SMB, above that will disable SMB
    , curve: "rapid-acting" // change this to "ultra-rapid" for Fiasp, or "bilinear" for old curve
    , useCustomPeakTime: true // allows changing insulinPeakTime
    , insulinPeakTime: 60 // number of minutes after a bolus activity peaks.  defaults to 55m for Fiasp if useCustomPeakTime: false
